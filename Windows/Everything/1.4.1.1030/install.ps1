...
if ($Arch -eq "ARM64") {
    throw "ARM64 architecture not supported for Everything"
} elseif ($Arch -eq "ARM") {
    throw "ARM architecture not supported for Everything"
} else {
    $InstallerName = "Everything-1.4.1.1030.x64-Setup.exe"
    $VendorUrl = "https://www.voidtools.com/Everything-1.4.1.1030.x64-Setup.exe"
}

$TempDir = "$env:TEMP\everything-install"
...
try {
    ...
    if ($WTF) {
        ...
        $manifestPath = "Windows/Everything/1.4.1.1030/manifest.json"
        ...
        # Run installer silently (NSIS installer uses /S)
