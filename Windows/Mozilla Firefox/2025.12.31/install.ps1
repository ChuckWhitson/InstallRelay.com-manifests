...
$Version = "2025.12.31"
$VendorUrl = "https://download.mozilla.org/?product=firefox-latest&os=win64&lang=en-US"
$InstallerName = "Firefox Setup.exe"
$TempDir = "$env:TEMP\firefox-install"
...
try {
    ...
    # Download installer directly from Mozilla
    $InstallerPath = Join-Path $TempDir $InstallerName
    
    if ($WTF) {
        Write-Host "[WTF] Would download installer from Mozilla..." -ForegroundColor Yellow
        ...
        Write-Host "[WTF] Would install using: $InstallerName -ms" -ForegroundColor Yellow
        Write-Host "[WTF] Installation test completed - no changes made" -ForegroundColor Green
    } else {
        Write-Host "Downloading installer from Mozilla..." -ForegroundColor Yellow
