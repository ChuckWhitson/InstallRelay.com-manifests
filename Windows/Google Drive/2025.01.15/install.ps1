...
$Version = "2025.01.15"
$VendorUrl = "https://dl.google.com/drive-file-stream/GoogleDriveSetup.exe"
$InstallerName = "GoogleDriveSetup.exe"
$TempDir = "$env:TEMP\google-drive-install"
...
function Verify-FileHashFromGCS {
    param(
        [string]$FilePath,
        [string]$ManifestPath,  # e.g., "Windows/Google Drive/2025.01.15/manifest.json"
        [string]$InstallerName  # e.g., "GoogleDriveSetup.exe"
    )
...
try {
    ...
    if ($WTF) {
        Write-Host "[WTF] Would download installer from Google..." -ForegroundColor Yellow
        ...
        Write-Host "[WTF] Would install using: $InstallerName --silent --desktop_shortcut --skip_launch_new --gsuite_shortcuts=false" -ForegroundColor Yellow
        Write-Host "[WTF] Installation test completed - no changes made" -ForegroundColor Green
    } else {
        Write-Host "Downloading installer from Google..." -ForegroundColor Yellow
        ...
        $manifestPath = "Windows/Google Drive/2025.01.15/manifest.json"
        ...
        $Process = Start-Process -FilePath $InstallerPath -ArgumentList "--silent", "--desktop_shortcut", "--skip_launch_new", "--gsuite_shortcuts=false" -Wait -PassThru -NoNewWindow
        if ($Process.ExitCode -eq 0 -or $Process.ExitCode -eq $null) {
            Write-Host "Installation completed successfully!" -ForegroundColor Green
            Write-Host "Please sign in with your Google account to start syncing." -ForegroundColor Cyan
        } else {
