...
if ($Arch -eq "ARM64") {
    $InstallerName = "PowerShell-7.5.0-win-arm64.msi"
    $VendorUrl = "https://github.com/PowerShell/PowerShell/releases/download/v7.5.0/PowerShell-7.5.0-win-arm64.msi"
} else {
    $InstallerName = "PowerShell-7.5.0-win-x64.msi"
    $VendorUrl = "https://github.com/PowerShell/PowerShell/releases/download/v7.5.0/PowerShell-7.5.0-win-x64.msi"
}

$TempDir = "$env:TEMP\powershell-install"
...
try {
    ...
    if ($WTF) {
        Write-Host "[WTF] Would download installer from GitHub..." -ForegroundColor Yellow
        ...
        Write-Host "[WTF] Would install using: msiexec.exe /i `"$InstallerPath`" /quiet /norestart" -ForegroundColor Yellow
        Write-Host "[WTF] Installation test completed - no changes made" -ForegroundColor Green
    } else {
        Write-Host "Downloading installer from GitHub..." -ForegroundColor Yellow
        ...
        if ($Process.ExitCode -eq 0) {
            ...
            Write-Host "PowerShell $Version is now available as 'pwsh'" -ForegroundColor Cyan
        } else {
