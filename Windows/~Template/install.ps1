...
if ($Arch -eq "ARM64") {
    $InstallerName = "app-name-arm64.exe"
    $VendorUrl = "https://example.com/downloads/app-name-arm64.exe"
} elseif ($Arch -eq "ARM") {
    $InstallerName = "app-name-arm.exe"
    $VendorUrl = "https://example.com/downloads/app-name-arm.exe"
} else {
    $InstallerName = "app-name-x64.exe"
    $VendorUrl = "https://example.com/downloads/app-name-x64.exe"
}

$TempDir = "$env:TEMP\app-name-install"
...
try {
    ...
    if ($WTF) {
        Write-Host "[WTF] Would download installer from vendor..." -ForegroundColor Yellow
        ...
        # TODO: Update install arguments based on your application's silent install parameters
        ...
        Write-Host "Downloading installer from vendor..." -ForegroundColor Yellow
        ...
        # TODO: Update manifest path to match your application
        ...
        #   msiexec.exe /i "installer.msi" /quiet /norestart - For MSI installers
