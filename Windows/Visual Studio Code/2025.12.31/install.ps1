...
$Version = "2025.12.31"
$VendorUrl = "https://code.visualstudio.com/sha/download?build=stable&os=win32-x64-user"
$InstallerName = "VSCodeUserSetup-x64.exe"
$TempDir = "$env:TEMP\vscode-install"
...
try {
    ...
    # Download installer directly from Microsoft
    $InstallerPath = Join-Path $TempDir $InstallerName
    
    if ($WTF) {
        Write-Host "[WTF] Would download installer from Microsoft..." -ForegroundColor Yellow
        ...
        Write-Host "[WTF] Would install using: $InstallerName /VERYSILENT /NORESTART" -ForegroundColor Yellow
        Write-Host "[WTF] Installation test completed - no changes made" -ForegroundColor Green
    } else {
        Write-Host "Downloading installer from Microsoft..." -ForegroundColor Yellow
        ...
        $manifestPath = "Windows/Visual Studio Code/2025.12.31/manifest.json"
