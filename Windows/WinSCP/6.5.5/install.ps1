...
$AppName = "WinSCP"
$Version = "6.5.5"
# WinSCP download URLs - try direct CDN download first, fallback to download page
$DirectDownloadUrl = "https://cdn.winscp.net/files/WinSCP-6.5.5-Setup.exe?secure=ubO8gkr1AVlUojxIVqTEBQ==,1767254791"
...
function Resolve-WinSCPDownloadUrl {
    ...
    try {
        ...
        $fallbackUrls = @(
            "https://cdn.winscp.net/files/WinSCP-6.5.5-Setup.exe?secure=ubO8gkr1AVlUojxIVqTEBQ==,1767254791",
            "https://winscp.net/eng/downloads/WinSCP-6.5.5-Setup.exe",
...
function Verify-FileHashFromGCS {
    param(
        [string]$FilePath,
        [string]$ManifestPath,  # e.g., "Windows/WinSCP/6.5.5/manifest.json"
        [string]$InstallerName  # e.g., "WinSCP-6.5.5-Setup.exe"
    )
...
try {
    ...
    if ($WTF) {
        ...
        Write-Host "[WTF] Would install using: $InstallerName /VERYSILENT /ALLUSERS /NORESTART" -ForegroundColor Yellow
        ...
        if (-not (Download-File -Url $actualDownloadUrl -Destination $InstallerPath)) {
            throw "Failed to download installer"
        }
        
        # Verify file hash against manifest hash value or .sha256 file in GCS
        $manifestPath = "Windows/WinSCP/6.5.5/manifest.json"
        ...
        $Process = Start-Process -FilePath $InstallerPath -ArgumentList "/VERYSILENT", "/ALLUSERS", "/NORESTART" -Wait -PassThru -NoNewWindow
