...
$AppName = "ImgBurn"
$Version = "latest"
$VendorUrl = "https://www.imgburn.com/index.php?act=download"
$InstallerName = "SetupImgBurn.exe"
$TempDir = "$env:TEMP\imgburn-install"
...
try {
    if ($WTF) {
        Write-Host "[WTF] Would install: $AppName" -ForegroundColor Green
    } else {
        Write-Host "Installing $AppName..." -ForegroundColor Green
    }
    
    # Download installer directly from ImgBurn website
    $InstallerPath = Join-Path $TempDir $InstallerName
    
    if ($WTF) {
        Write-Host "[WTF] Would download installer from ImgBurn website..." -ForegroundColor Yellow
        ...
        Write-Host "[WTF] Would install using: $InstallerName /S /nocandy" -ForegroundColor Yellow
        Write-Host "[WTF] Installation test completed - no changes made" -ForegroundColor Green
    } else {
        Write-Host "Downloading installer from ImgBurn website..." -ForegroundColor Yellow
        ...
        $manifestPath = "Windows/ImgBurn/latest/manifest.json"
        ...
        # Run installer silently (with /nocandy to avoid adware)
        Write-Host "Running installer..." -ForegroundColor Yellow
        $Process = Start-Process -FilePath $InstallerPath -ArgumentList "/S", "/nocandy" -Wait -PassThru -NoNewWindow
